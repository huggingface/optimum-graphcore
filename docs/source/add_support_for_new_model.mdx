...
2. Create a "pipelined" version of the original class, to do that:
  ...
  - Implement the [`~modeling_utils.PipelineMixin.deparellize`] method, which takes a parallelized instance of the model back to its original version. This is needed to make sure both the original and pipelined versions can share the same state dict.
