...
The configuration parameter [`IPUConfig.layers_per_ipu`] specifies the number of layers that will be put on each IPU for pipelined execution during training. There is an equivalent parameter for inference, [`IPUConfig.inference_layers_per_ipu`].
