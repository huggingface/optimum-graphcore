...
- [`IPUTrainer`]: the trainer class that takes care of compiling the model to run on IPUs, and of performing training and evaluation, check [here](trainer.mdx) for more information..
- [`IPUConfig`]: the class that specifies attributes and configuration parameters to compile and put the model on the device, check [here](ipu_config.mdx) for more information.
