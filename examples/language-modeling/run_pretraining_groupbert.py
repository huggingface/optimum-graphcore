...
tf_check_min_version("4.13.0.dev0")
...
class DataTrainingArguments:
    ...
    data_dir: Optional[str] = field(
        default=None, metadata={"help": "The path to a local folder containing the data files."}
    )
...
def log_args(training_args, model_args, data_args):
...
def set_profile_flags(profile, execution):
    """
    Set poplar flags for profiling pytorch model on the IPU
    """
    ...
    if execution or profile:
        report_dict = {
            "autoReport.outputExecutionProfile": "true" if execution else "false",
            "debug.instrument": "true" if execution else "false",
...
def prepare_callbacks(model_args, config):
    ...
    if model_args.wandb:
        ...
        wandb.init(project="POPTORCH-GROUPBERT", config=config, name=model_args.wandb_name)
...
def get_groupbert_model(model_args, config):
    # Distributed training:
    # The .from_pretrained methods guarantee that only one local process can concurrently
    # download model & vocab.

    if model_args.model_name_or_path:
        model = GroupBertForPreTraining.from_pretrained(
...
def process_the_dataset(raw_datasets, data_args, training_args, tokenizer):
