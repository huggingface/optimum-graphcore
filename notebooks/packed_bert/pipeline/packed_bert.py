...
from utils.packing.qa_utils import preprocess_packed_qa, postprocess_packed_qa_predictions
from utils.packing.dataset_creator import PackedDatasetCreator
from models.modeling_bert_packed import PipelinedPackedBertForSequenceClassification, PipelinedPackedBertForQuestionAnswering
...
def prepare_inference_dataloader(ipu_config, dataset, batch_size, mode='async_rebatched'):
    return poptorch.DataLoader(ipu_config.to_options(for_inference=True),
        dataset,
...
class PackedBertTextClassificationPipeline:
    ...
    def __init__(self,
        ...
        self.model = PipelinedPackedBertForSequenceClassification(config).from_pretrained(
            self.model_ckpt, config=config).half()
        ...
        pck_compile_data = PackedDatasetCreator(
            ...
            problem_type = self.problem_type).create()
        ...
        self.poplar_executor = get_poplar_executor(
            self.model, self.ipu_config, c_batch)

        

    def preprocess_function(self, examples):
        if self.sentence_2_key:
            return self.tokenizer(examples['text'], examples['text_2'], truncation=True, max_length=self.max_seq_length)
        else:
...
class PackedBertQuestionAnsweringPipeline:
    ...
    def __init__(self,
        ...
        self.model = PipelinedPackedBertForQuestionAnswering(config).from_pretrained(
            self.model_ckpt, config=config).half()
            
        compile_data = Dataset.from_dict({'id': np.array([str(i) for i in range(self.gbs)]).astype('<U32'),
                                          'question':["Do trees have leaves in the wintertime?."] * self.gbs,
                                          'context' :["Most trees leaves fall off after the autumn season. However, evergreen trees keep their leaves through winter."] * self.gbs})
    ...
    def predict(self, questions, contexts):
        ...
        dataset = Dataset.from_dict({'id': np.array([str(i) for i in range(len(questions))]).astype('<U32'), 'question': questions, 'context':contexts})
        ...
        formatted_predictions = [
            {"id": k, "prediction_text": v} for k, v in final_preds.items()
        ]
