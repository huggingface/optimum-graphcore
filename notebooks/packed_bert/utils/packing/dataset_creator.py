...
class PackedDatasetCreator:
    def __init__(self,
        ...
        if problem_type in supported_problem_types:
            ...
            logger.warning(f'Unsupported problem type given - attempting to detect from number of labels (default 1, unless specifically passed). \
            ...
            if 'start_positions' in tokenized_dataset:
                ...
                self.problem_type = 'single_label_classification' if self.num_labels == 1 else 'multi_label_classification'

        # Verify the task
        if not training and not validation and not inference:
            logger.error("One of 'training', 'validation' or 'inference' must be set to True when calling PackedDatasetCreator.")
            raise Exception
        ...
        if problem_type == 'single_label_classification':
            ...
            if not self.inference:
                self.unpacked_labels = tokenized_dataset[custom_label_key]
                self.packed_labels = -100 * np.ones((self.total_num_packs, self.max_seq_per_pack, self.num_labels), dtype=int) 
            else:
                self.packed_labels = None
    ...
    def create(self):
        ...
        for i in range(len(strategy_repeat_count)):
            ...
            for _ in range(strategy_repeat_count[i]):
                ...
                attention_mask_pack = list(itertools.chain(*[itertools.repeat(n+1, len(self.unpacked_attention_mask[v])-skip_cls) for n,v in enumerate(inds)]))
                token_type_ids_pack = list(itertools.chain(*[self.unpacked_token_type_ids[x][skip_cls:] for x in inds]))
                position_ids_pack   = list(itertools.chain(*[range(skip_cls, len(self.unpacked_attention_mask[v])) for n,v in enumerate(inds)]))
                ...
                if self.problem_type == 'question_answering':
                    if self.training:
                        start_positions_pack = [max(self.unpacked_start_positions[v] + positions_offset[n], 0) for n,v in enumerate(inds)]
                        end_positions_pack   = [max(self.unpacked_end_positions[v] + positions_offset[n], 0) for n,v in enumerate(inds)]
                ...
                if skip_cls:
                    self.packed_input_ids[pack_index, -self.max_seq_per_pack:] = [self.unpacked_input_ids[0][0] for _ in range(self.max_seq_per_pack)]
                    self.packed_attention_mask[pack_index, -self.max_seq_per_pack:] = list(range(1, self.max_seq_per_pack+1))
