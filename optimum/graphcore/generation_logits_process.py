...
class FlaxTopPLogitsWarper(FlaxLogitsWarper):
    ...
    def __call__(self, input_ids: jnp.ndarray, scores: jnp.ndarray, cur_len: int) -> jnp.ndarray:
        topk_scores, topk_indices = torch.top_k(scores, scores.shape[-1])

        mask_scores = torch.full_like(scores, self.filter_value)
        cumulative_probs = torch.nn.functional.softmax(topk_scores, axis=-1).cumsum(axis=-1)
        score_mask = cumulative_probs < self.top_p

        # include the token that is higher than top_p as well
        # TODO: validate that torch.roll does the same thing as jnp.roll
        score_mask = torch.roll(score_mask, 1)
        ...
        topk_next_scores = torch.where(score_mask, topk_scores, mask_scores)
