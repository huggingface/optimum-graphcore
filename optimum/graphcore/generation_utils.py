...
# def flatten_dict(d: Dict[str, torch.Tensor]) -> List[Tuple[torch.IntTensor, torch.ByteTensor, torch.Tensor]]:
#     hash_ = torch.tensor(id(d), dtype=torch.long)
#     return [(hash_, torch.tensor(key.encoder("utf-8"), dtype=torch.bytes), value) for key, value in d.items()]


def as_list(state):
    return [getattr(state, field.name) for field in fields(state)]
...
class State(list):
    ...
    def __post_init__(self):
        self._field_name_to_idx = None
        self._pytree_specs = None
        self._list = []
        self._create_field_name_to_index_mapping()

    def _create_field_name_to_index_mapping(self):
        ...
        for field in fields(self):
            ...
            if not isinstance(value, torch.Tensor):
                flatten, specs = tree_flatten(value)
                shift = len(flatten)
                self._pytree_specs[index] = (specs, shift)
                self._field_name_to_idx[field.name] = index
                self._list += flatten
            else:
    ...
    def from_list(cls, l, specs):
        ...
        while index < len(l):
            name = names.pop(0)
            if index in specs:
                field_specs, shift = specs[index]
                kwargs[name] = tree_unflatten(l[index: index + shift], field_specs)
                index += shift
            else:
                kwargs[name] = l[index]
                index += 1
...
class IPUGenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(self, input_ids, model_kwargs):
        ...
        model_kwargs["encoder_outputs"] = dict(self.get_encoder()(input_ids, return_dict=True, **encoder_kwargs))
        return model_kwargs
...
#         return model
    ...
    def _greedy_search(
        ...
        def greedy_search_body_fn(*state):
            ...
            logits = model_outputs[0][:, -1] # model_outputs.logits[:, -1]
