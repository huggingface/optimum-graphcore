...
class PipelinedBartForConditionalGeneration(BartForConditionalGeneration, PipelineMixin):
    def parallelize(self):
        ...
        if self.config.embedding_serialization_factor > 1:
            ...
            # TODO: focus on embed_tokens and embed_positions later.
            # self.model.encoder.embed_tokens = SerializedEmbedding(
            #     self.model.encoder.embed_tokens, self.config.embedding_serialization_factor
            # )
        ...
        # TODO: how to support layer_ipu for encoder / decoder when # encoders != # decoders?
        ...
        self.lm_head = poptorch.BeginBlock(self.lm_head, "LM Head Output", ipu_id=ipu)
        logger.info("-----------------------------------------------------------")
        return self
