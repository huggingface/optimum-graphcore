...
FB_TO_HF_MAP = {
    "downsample_layers.0.0.weight": "convnext.embeddings.patch_embeddings.weight",
...
def run_test():
    for key in FB_TO_HF_MAP.keys():
        assert (
            fb_to_hf_name(key) == FB_TO_HF_MAP[key]
        ), f"Not matching for {key}. {fb_to_hf_name(key)} != {FB_TO_HF_MAP[key]}"
